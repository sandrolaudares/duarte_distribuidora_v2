<script lang="ts">
  ...
  let steps = [
    ...
    // { title: 'Passo 3', status: 'Pending' },
  ]
  $:{
    if(infos.tenantName && infos.subdomain && infos.name && infos.email && infos.password && infos.confirmPassword) {
      steps[0].status = 'Concluido'
    }
    if(infos.phone && infos.cep && infos.street && infos.neighborhood && infos.number && infos.city) {
      steps[1].status = 'Concluido'
    }
  }
  let currentActive: number = 1
  $: stepAtivo = steps[currentActive - 1]
  let progressBar: { handleProgress: (stepIncrement: number) => void }
      ...
      >
        {#if stepAtivo.title === 'Insira os dados'}
          ...
          <button
            ...
            class="disabled:bg-base-300 group relative flex w-full items-center justify-center gap-2 rounded-md border border-transparent bg-primary px-4 py-2 text-sm font-medium hover:bg-opacity-70 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
          >
            Proximo passo
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {:else if stepAtivo.title === 'Informações adicionais'}
          ...
          <button
            disabled={isLoading || (steps[0].status !== 'Concluido' && steps[1].status !== 'Concluido')}
            type="submit"
